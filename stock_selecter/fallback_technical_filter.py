...
def run_technical_filter(threshold=10, return_reasons=False):
    ...
    df = all_features[all_features["date"] == date].copy()

    if df.empty:
        logger.warning(f"⚠️ No feature data available for {date}")
        return (pd.DataFrame(), {}) if return_reasons else pd.DataFrame()

    # Conditions
    cond1 = df["sma_short"] > df["sma_long"]
